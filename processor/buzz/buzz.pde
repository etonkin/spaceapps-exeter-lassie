...
void setup() {
  //Table table = loadTable("/home/em/Work/NASA/2015-space-app-challenge/spaceapps-exeter-working/trunk/qr-create/first-correct-dataset/dataset.csv", "header");
  size(800,800);
  ...
  sourceb = loadImage("/home/em/Work/NASA/2015-space-app-challenge/spaceapps-exeter-lassie/resources/buzz-face.png"); 
  sourceb.resize(1300,1300);
   ...
   for (int x = 0; x < source.width; x++) {
    for (int y = 0; y < source.height; y++ ) {
      ...
      if (brightness(source.pixels[loc]) > threshold) {
        ...
        if(brightness(pg.pixels[loc])>threshold){ 
    ...
    for (int y = 0; y < source.height-vertoffset*2; y++ ) {
      ...
      if (brightness(source.pixels[loc]) > threshold) {
        ...
        if (x<iconwidth) {
           if (y<iconheight){
             int loc2 = x + y*sourceb.width;
             if (brightness(sourceb.pixels[loc2])<250){
